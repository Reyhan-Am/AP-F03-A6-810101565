...
Request *parseRawReq(char *headersRaw, size_t length)
{
  ...
  try
  {
    ...
    {
      if (!headersRaw[i])
        throw Exception("Unsupported binary data in request.");
    }
    size_t realBodySize =
    ...
    for (size_t headerIndex = 0; headerIndex < headers.size(); headerIndex++)
    {
      ...
      case REQ:
      {
        ...
        if (R.size() != 3)
        {
          throw Exception("Invalid header (request line)");
        }
        req = new Request(R[0]);
        req->setPath(R[1]);
        ...
        if (toLowerCase(R[0]) == toLowerCase("Content-Disposition"))
        {
          ...
          for (size_t i = 0; i < A.size(); i++)
          {
            ...
            else if (attr.size() == 1)
            {
              ...
              throw Exception("Invalid body attribute");
          }
        }
        else if (toLowerCase(R[0]) == toLowerCase("Content-Type"))
        {
          if (toLowerCase(R[1]) == toLowerCase("application/octet-stream"))
            shouldBeEmpty = true;
                   ...
                   toLowerCase("text"))
            throw Exception("Unsupported file type: " + R[1]);
        }
      }
      break;
      case BODY_BODY:
  ...
  {
    throw Exception("Error on parsing request");
  }
  return req;
}
...
Response *ShowFile::callback(Request *req, string msg)
{
  Response *res = new Response;
  res->setHeader("Content-Type", fileType);
  res->setBody(readFile(filePath.c_str()));
  if (sessionData.find(req->getSessionId()) != sessionData.end())
  {
